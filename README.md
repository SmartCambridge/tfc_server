...
Rita data is available via a web-services API.

Currently implemented are:

## /api/console/status

This returns the current status of all the Rita modules known to the server-side Console module. Every
module in the Rita system sends periodic status updates on a defined EventBus address, to be
noted by the Console module.

Note that each module provides its *own* definition of how long it should be before a module is flagged
as AMBER or RED, as this may vary by module.

Each module status format is:
```
 { "module_name":"dataserver", // the config() module_name of the module reporting it's status
   "module_id":"vix",          // the config() module_id assigned to this module
   "status":"UP",              // currently only "UP" is supported. Down is recognized by aged timestamp
   "status_amber_seconds":15,  // how aged the timestamp can be before this module should be flagged as AMBER
   "status_red_seconds":25,    // how aged the timestamp can be before this module should be flagged as RED
   "ts":1475486840             // utc timestamp (seconds) when module sent this status to Console
 }
```

E.g.
```
{ "module_name":"console",   // module_name of module providing this API
  "module_id":"A",           // module_id of module providing this API
  "status":[                 // JsonArray of reported statuses (as above)
             {"module_name":"dataserver","module_id":"vix","status":"UP","status_amber_seconds":15,"status_red_seconds":25,"ts":1475486840},
             {"module_name":"msgfiler","module_id":"zone_cambridge","status":"UP","status_amber_seconds":15,"status_red_seconds":25,"ts":1475486842},
             ...
           ]
}
```
